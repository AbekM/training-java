...
public class QuadrilateralTest {
    ...
    static Stream<Arguments> testAreaCases() throws IOException {
        Iterator<String> areas = Files.readAllLines(Paths.get("src/test/java/com/epam/training/Asilbek_Makhmudov/figures/resources/quadrilateral-area.txt")).iterator();
        return quadrilaterals()
                .map(q -> Arguments.of(q, parseDouble(areas.next())));
    }

    static Stream<Arguments> testToStringCases() throws IOException {
        Iterator<String> areas = Files.readAllLines(Paths.get("src/test/java/com/epam/training/Asilbek_Makhmudov/figures/resources/quadrilateral-string.txt")).iterator();
        return quadrilaterals()
                .map(q -> Arguments.of(q, areas.next()));
    }

    static Stream<Arguments> testPointsCases() throws IOException {
        Iterator<String> areas = Files.readAllLines(Paths.get("src/test/java/com/epam/training/Asilbek_Makhmudov/figures/resources/quadrilateral-points.txt")).iterator();
        return quadrilaterals()
                .map(q -> Arguments.of(q, areas.next()));
    }

    static Stream<Arguments> testLeftmostCases() throws IOException {
        Iterator<String> expected = Files.readAllLines(Paths.get("src/test/java/com/epam/training/" +
                "Asilbek_Makhmudov/figures/resources/quadrilateral-leftmost.txt")).iterator();
        return quadrilaterals()
